...
- `raw/chunk_{gpu3000,cpu1000}_{summary,percell}.csv` — **authoritative raw aggregate**, computed
...
1. **Each implementation's batch/chunk knob is a real dial for fit *time*** — up to ~25× within one
   ...
   VRAM (~2.7–3.6× NVML); jamica's GPU memory is flat (~5.4 GiB) on its chunked path.
...
3. **Fit times are wall time to a fixed iteration budget (GPU 3000 / CPU 1000), NOT time to an
   ...
   budget (e.g. on GPU at the largest chunk: pyamica always 3000, scott-huberty 784–1654, pAMICA
...
- **NVML is the headline VRAM** (framework-neutral whole-GPU peak). Per-framework allocator counters
  (JAX `peak_bytes_in_use`, torch `max_memory_allocated`) understate the footprint ~1.4–3.4× and are
  not comparable across frameworks.
- **jamica is measured on its chunked path** (`amica_python_jax_chunked`). Its full-batch key
  (`amica_python_jax`, `chunk_size=None`) is a separate program (~11.4 GiB NVML at the same GPU time;
  much slower + 19.8 GiB on CPU) — discussed only in the memory note.
- **`262144` is the largest tested chunk, not "full-batch"** — recordings are 785k–1.36M samples, so
  262144 is ~15–33% of the data.

## Provenance
- GPU @3000: `/scratch/yorguin/iter_ladder/gpu/c<chunk>_i3000/` (Trillium H100, 20–25 subj/cell).
